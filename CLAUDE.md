...
- **Обновление токена в шаблоне не реализовано.** `refreshToken` сохраняется, `/auth/refresh-token` лежит в `publicRoutes.js`, но `checkAuthTokens` в `store/user.js` только отменяет запрос при отсутствующем токене — срок жизни не проверяется. Схема обновления зависит от бэкенда, поэтому дописывается в проекте. Точка расширения одна — `checkAuthTokens`, в интерцептор логику не растаскиваем.
...
- **TODO-комментарии запрещены** (`sonarjs/todo-tag`). Незакрытое место описывается словами: что не сделано и от чего зависит — см. `checkAuthTokens` в `store/user.js`.
