...
5. CORS на бэкенде — он должен отдавать `Access-Control-Allow-Credentials`, иначе refresh-кука не доедет

## Что внутри

- **Приватный роутинг по умолчанию** — публичный роут помечается `meta: { public: true }`, guard работает в обе стороны
- **axios** с baseURL из env, подстановкой токена в request-интерцепторе и разбором ошибок в response
- **Авторизация на httpOnly-куке** — `accessToken` только в памяти Pinia, refresh-токен в куке недоступен для JS; при 401 интерцептор сам обновляет токен и повторяет запрос
- **29 тестов** на guard, стор, интерцептор, обработчик ошибок, страницу и layout — эталоны для дальнейших
